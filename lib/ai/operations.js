function createOperationDomain(scope) {
  ...
  // Callers replace the user's selection with whatever comes back, so an empty
  // edit is not a successful one -- it silently deletes their text. Refuse it
  // here rather than in each caller.
  function editResult(edited) {
    const next = String(edited || '').trim();
    if (!next) return { ok: false, error: 'The model returned an empty edit, so your text was left unchanged.' };
