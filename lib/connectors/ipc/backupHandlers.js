function registerBackupHandlers(ipcMain, deps) {
  const {
    ...
    withIndexVaultLock = async (_vaultId, fn) => await fn(),
    vaultWatcher = { refresh() {} },
  } = deps;

  async function rescanImportedVault(vaultId, label) {
    return await withIndexVaultLock(vaultId, async () => {
      const loaded = await store.loadVault(vaultId);
      await Promise.resolve(runOptionalSearchIndexTask(label, () => idx.rescanVault(vaultId, loaded.notes || [])));
      return loaded;
    });
  }
  ...
  ipcMain.handle('mn:exportBackup', wrap(async (options = {}) => {
    ...
    // exportBackup already serialized the payload and measured it. Stringifying
    // it again here would put a second multi-megabyte string in memory beside
    // the first, and would report a size the file does not have.
    const { payload, text, sizeBytes, noteCount, canvasCount } = await store.exportBackup(options || {});
    await store.atomicWriteFile(result.filePath, text, 'utf8');
    const warnings = [...(payload.warnings || [])];
    // An incomplete backup must not move the last-backup date: a green
    // "backed up" indicator over a file missing notes is the lie being closed.
    if (!warnings.length) {
      try { await store.recordBackupExport?.(payload.exportedAt); }
      catch (error) { warnings.push({ type: 'backup-status', message: error.message || String(error) }); }
    }
    return {
      canceled: false,
      filePath: result.filePath,
      exportedAt: payload.exportedAt,
      vaultCount: payload.vaults.length,
      noteCount,
      canvasCount,
      sizeBytes,
      warnings,
  ...
  ipcMain.handle('mn:importBackup', wrap(async (options = {}) => {
    ...
    for (const vault of imported.importedVaults || []) {
      await rescanImportedVault(vault.id, 'rescan imported vault');
    }
  ...
  ipcMain.handle('mn:applyMarkdownImport', wrap(async (options = {}) => {
    ...
    await rescanImportedVault(options.vaultId, 'rescan Markdown import');
    return imported;
  }));
