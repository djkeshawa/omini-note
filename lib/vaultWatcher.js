...
// window it replaces.
//
// Windows needs a wider tolerance. Its system clock ticks at ~15.6ms while
// NTFS records file times from a finer source, so mtime routinely reads tens
// of milliseconds ahead of the Date.now() we stamped a moment earlier. At 2ms
// that made a save intermittently look like somebody else's edit — the user's
// own writes surfacing as external changes, and the Windows release
// validation failing at random.
//
// Kept as tight as each platform allows: every extra millisecond is a window
// in which a real outside edit to the same file could be mistaken for ours.
...
function createVaultWatcher({
  ...
  async function isExplainedByOurWrite(vaultId, fileName) {
    ...
    try {
      ...
      return info.mtimeMs <= doneAt + skewMs;
    } catch {
