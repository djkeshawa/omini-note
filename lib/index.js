...
const { searchWithFallback } = require('./searchQuery');
...
function prepareStatements() {
  return {
    ...
    vectorChunkByRowid: db.prepare(`
      ...
      WHERE c.rowid = ? AND c.content_hash = n.content_hash
    `),
  };
}
...
function search(vaultId, query, limit = 50) {
  ...
  return searchWithFallback(query, (fts) => {
    try {
      return db.prepare(`
        SELECT
          f.note_id AS id,
...
function searchDetailedRows(vaultId, query, limit = 50) {
  ...
  return searchWithFallback(query, (fts) => db.prepare(`
    SELECT
      f.note_id AS id,
      f.title AS title,
      snippet(notes_fts, 3, '<mark>', '</mark>', '…', 16) AS snippet,
...
function setNoteEmbeddings(vaultId, note, embeddings, model, expectedContentHash = noteContentHash(note)) {
  if (!db) init();
  ...
  const tx = db.transaction(() => {
    const current = db.prepare('SELECT content_hash AS contentHash FROM notes WHERE id = ?').get(key);
    if (!current || current.contentHash !== expectedContentHash) {
      return { committed: false, currentContentHash: current?.contentHash || null };
    }
    removeEmbeddings(vaultId, note.id);
    const insChunk = db.prepare(`
      INSERT INTO embedding_chunks (note_id, vault_id, chunk_idx, text, model, content_hash, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
    const insVec = db.prepare('INSERT INTO note_embeddings (rowid, embedding) VALUES (?, ?)');
    for (let i = 0; i < chunks.length; i++) {
      const r = insChunk.run(key, vaultId, i, chunks[i], embeddingModelKey(model), expectedContentHash, now);
    ...
    return { committed: true, contentHash: expectedContentHash };
  });
  return tx();
}
...
function notesNeedingEmbeddings(vaultId, model) {
  ...
  return db.prepare(`
    ...
    AND NOT EXISTS (
      ...
      WHERE c.note_id = n.id AND c.model = ? AND c.content_hash = n.content_hash
    )
...
function indexHealth(vaultId, model = DEFAULT_EMBED_MODEL) {
  ...
  const embed = db.prepare(`
    ...
    FROM embedding_chunks c
    JOIN notes n ON n.id = c.note_id
    WHERE c.vault_id = ? AND c.model = ? AND c.content_hash = n.content_hash
...
module.exports = {
  ...
  // Embeddings
  chunkNote, noteContentHash, setNoteEmbeddings, removeEmbeddings,
