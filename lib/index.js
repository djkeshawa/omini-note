...
function close() {
  if (db) { db.close(); db = null; statements = null; }
}
...
function removeVault(vaultId) {
  ...
  const ids = statements.noteIdsByVault.all(vaultId).map(r => r.id);
  const tx = db.transaction(() => {
...
function backlinks(vaultId, title, limit = 100) {
  ...
  return db.prepare(`
    ...
    LIMIT ?
  `).all(vaultId, title, title, Math.max(1, Math.min(Number(limit) || 100, 500))).map(r => ({ ...r, id: noteIdFromKey(vaultId, r.id) }));
}
...
module.exports = {
  ...
  search, searchDetailed, lexicalContextSearch, backlinks, notesByTag, tagCounts,
  // Embeddings
