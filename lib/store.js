...
async function loadConfig() {
  ...
  if (cfg && cfg.vaults && cfg.vaults.length) return await repairConfigVaults(cfg);
  // First run: seed
...
async function createVaultRecord(cfg, name, options = {}) {
  ...
  const firstNoteTitle = isNovelist ? 'Manuscript' : `Welcome to ${cleanName}`;
  const firstNoteTags = isNovelist ? ['novel-manuscript'] : [];
  const firstNoteBody = isNovelist
  ...
  await fsp.writeFile(
    ...
    serializeFrontMatter(
      { id: firstNoteId, title: firstNoteTitle, date: new Date().toISOString(), tags: firstNoteTags, pinned: isNovelist },
      firstNoteBody
    ),
    'utf8'
  );
