...
const { noteQuarantine, recoverOrSeedConfig, getConfigRecovery } = require('./storage/configRecovery').createConfigRecovery({ fsp, path, root: ROOT, maxMetaBytes: MAX_JSON_FILE_BYTES, writeConfigSnapshot, firstRunSeed });
...
async function initializeConfig() {
  ...
  const cfg = await readJsonSafe(CONFIG_FILE, null, { onQuarantine: noteQuarantine });
  if (cfg && cfg.vaults && cfg.vaults.length) {
    configCache = cloneData(await repairConfigVaults(cfg));
    configCacheRepaired = true;
    return;
  }
  configCache = cloneData(await recoverOrSeedConfig(cfg ? 'empty' : 'missing')); // adopt vault folders before seeding; a quarantined read overrides this to 'unreadable'
  configCacheRepaired = true;
}
...
const preferencesRepository = createPreferencesRepository({
  ...
  assertByteLength, getConfigRecovery,
  maxJsonWriteBytes: MAX_JSON_WRITE_BYTES,
