...
async function readJsonSafe(file, fallback) {
  try {
    ...
    if (stat.size > MAX_JSON_FILE_BYTES) throw new Error(`JSON file too large: ${path.basename(file)}`);
...
async function createVaultRecord(cfg, name, options = {}) {
  ...
  const finalSlug = allocateVaultSlug(cfg, cleanName);
...
async function restoreDeletedNote(vaultId, trashId) {
  ...
  if (await readExistingFile(noteFile(v.slug, noteId))) {
    ...
    for (let i = 0; i < 1000; i++) {
      const candidate = validateNoteId(`${baseId}_restored_${Date.now().toString(36)}_${i}`);
  ...
  try {
    for (const source of data.vaults) {
      ...
      for (const note of Array.isArray(source.notes) ? source.notes : []) {
        ...
        const id = validateNoteId(note.id || `n_${Date.now().toString(36)}`);
        const text = serializeFrontMatter({
          id,
          title: cleanString(note.title, 'Untitled'),
          date: note.date || new Date().toISOString(),
      ...
      for (const canvas of Array.isArray(source.canvases) ? source.canvases : []) {
        ...
        await atomicWriteFile(canvasFile(stagingSlug, next.id), JSON.stringify(next, null, 2), 'utf8');
      }
      imported.push(v);
    }
    ...
    for (const item of staged) {
      try {
        await fsp.rename(item.stagingDir, item.finalDir);
      } catch (e) {
...
async function setPrefs(patch) {
  ...
  if (cleanPatch.tweaks) {
    ...
    cfg.tweaks = { ...(cfg.tweaks || {}), ...cleanMergePatch(cleanPatch.tweaks) };
  }
  if (cleanPatch.aiConfig) {
    if (!isPlainObject(cleanPatch.aiConfig)) throw new Error('Invalid AI config patch');
    cfg.aiConfig = { ...(cfg.aiConfig || {}), ...cleanMergePatch(cleanPatch.aiConfig) };
  }
