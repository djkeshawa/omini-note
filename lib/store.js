  ...
  try {
    for (const source of data.vaults) {
      ...
      const cleanMeta = sanitizeVaultMetaPatch({
        ...
        novelistAiConfig: source.meta?.novelistAiConfig || null,
      });
    ...
    for (const item of staged) {
      if (fs.existsSync(item.finalDir)) throw new Error('Vault import target already exists: ' + item.vault.slug);
