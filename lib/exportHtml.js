...
async function renderBlocks(body, ctx = {}) {
  ...
  const flushParagraph = async () => {
    ...
    paragraph = [];
    if (text) parts.push(`<p>${await renderInline(text, ctx)}</p>`);
  };
  const closeList = () => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^>\s?/.test(trimmed)) {
      await flushParagraph(); closeList();
      parts.push(`<blockquote>${await renderInline(trimmed.replace(/^>\s?/, ''), ctx)}</blockquote>`);
      continue;
    }
    if (/^\|.*\|$/.test(trimmed)) {
      await flushParagraph(); closeList();
    ...
    const listItem = line.match(/^(\s*)-\s+(\[( |x|X)\]\s+)?(.*)$/);
    if (listItem) {
      ...
      parts.push(`<li${cls} style="margin-left:${depth * 20}px">${marker}${await renderInline(listItem[4], ctx)}</li>`);
      continue;
    }
