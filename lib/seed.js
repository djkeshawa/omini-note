...
const { localDateKey } = require('../src/shared/dateUtils.js');
const { shortcutLabel } = require('../src/platform/shortcutFormat.js');

const SEED_TAGS = [
  { name: 'welcome', hue: 200 },
];

// The welcome note prints this machine's real accelerators, so it never tells a
// Linux user about a key their keyboard does not have.
function seedShortcut(id) {
  return shortcutLabel(id, process.platform, { compact: true });
}

// Every action is named inside an inline code span mid-sentence on purpose: a
// line-initial `- [ ]` or a complete wiki link in this body would make
// isOnboardingNote() read its own seed as user-authored work.
...
function onboardingDate(options = {}) {
  return options.date || localDateKey(options.now || Date.now());
}
