...
function isChildItem(item = {}) {
  return Boolean(item.parentItem) || item.itemType === 'note' || item.itemType === 'attachment';
}

function parentFirst(items) {
  // Boolean, not the parentItem string it used to fall out as: two children of
  // different parents compared as "different", so compare(a,b) and compare(b,a)
  // were both 1. An inconsistent comparator leaves the order unspecified, so
  // results with several child items came back in an arbitrary order.
  return [...items].sort((a, b) => {
    const aChild = isChildItem(a);
    const bChild = isChildItem(b);
    if (aChild !== bChild) return aChild ? 1 : -1;
    return 0;
  });
}

function topLevelDocuments(items) {
  return (Array.isArray(items) ? items : [])
    .map(normalizeItem)
    .filter(item => item.key && !item.parentItem && item.itemType !== 'note' && item.itemType !== 'attachment');
}
...
async function list(input = {}) {
  const limit = cleanLimit(input.limit, 20);
  const items = await fetchJson('/users/0/items', {
    itemType: '-attachment',
    sort: 'dateModified',
    direction: 'desc',
  ...
  return {
    results: topLevelDocuments(items).slice(0, limit),
  };
}
