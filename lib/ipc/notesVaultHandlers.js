...
function createNotesVaultHandlers(deps) {
  const { store, idx, ai, withIndexVaultLock, runOptionalSearchIndexTask } = deps;
  ...
  return {
    ...
    async saveNote(payload) {
      ...
      const saved = await withIndexVaultLock(vaultId, async () => {
      ...
      return ok({ note: saved, updatedAt: saved.diskModifiedAt || saved.modifiedAt || null });
    },
