  ...
  return {
    ...
    async saveNote(payload) {
      ...
      const { saved, linkedNoteUpdates, linkedNoteRename } = await withIndexVaultLock(vaultId, async () => {
        ...
        // in its new form.
        onVaultMutated(vaultId, [result.id, ...updates.map(note => note.id)]);
        return {
          saved: result,
    ...
    async deleteNote(payload) {
      const { vaultId, noteId, noteSnapshot, options } = validateNoteDeleteRequest(payload);
