...
function cleanExpectedRevision(value) {
  ...
  if (!REVISION_RE.test(revision)) throw validationError('Invalid expected revision');
  return revision;
}

function cleanExpectedModifiedAt(value) {
  if (value == null || value === '') return null;
  const date = new Date(value);
  if (!Number.isFinite(date.getTime())) throw validationError('Invalid expected modification time');
  return date.toISOString();
}

function validateWriteOptions(value, record = null) {
  const input = value == null ? {} : assertPlainObject(value, 'write options');
  const options = {};
  if (Object.prototype.hasOwnProperty.call(input, 'expectedRevision')) {
    options.expectedRevision = cleanExpectedRevision(input.expectedRevision);
  } else if (isPlainObject(record) && Object.prototype.hasOwnProperty.call(record, 'diskRevision')) {
    options.expectedRevision = cleanExpectedRevision(record.diskRevision);
  }
  if (Object.prototype.hasOwnProperty.call(input, 'expectedModifiedAt')) {
    options.expectedModifiedAt = cleanExpectedModifiedAt(input.expectedModifiedAt);
  }
  return options;
}
...
function validateNoteSaveRequest(payload) {
  ...
  return {
    ...
    noteId,
    note,
    options: validateWriteOptions(input.options, note),
  };
}

function validateNoteDeleteRequest(payload) {
  const input = assertPlainObject(payload);
  const noteSnapshot = isPlainObject(input.noteSnapshot) ? { ...input.noteSnapshot } : null;
  if (noteSnapshot && Object.prototype.hasOwnProperty.call(noteSnapshot, 'body')) {
    noteSnapshot.body = cleanNoteContent(noteSnapshot.body);
  }
  return {
    vaultId: cleanId(input.vaultId, 'vault id'),
    noteId: cleanId(input.noteId, 'note id'),
    noteSnapshot,
    options: validateWriteOptions(input.options, noteSnapshot),
  };
}
