function createFilesystem({ fs, path, maxJsonReadBytes, maxJsonWriteBytes }) {
  ...
  // Returns the path the broken file was set aside as, so a caller that can
  // rebuild the lost state (config recovery) can name the file for the user.
  ...
  // options.onQuarantine is notified with the `.broken.<stamp>` path when this
  // read had to set the file aside. The return contract is unchanged: the
  // parsed value, or the fallback.
  async function readJsonSafe(file, fallback, options = {}) {
    const reportQuarantine = (broken) => {
      if (broken && typeof options.onQuarantine === 'function') options.onQuarantine(broken);
    };
    let handle = null;
    try {
      ...
      if (error.code === 'ELOOP') {
        reportQuarantine(await quarantineBrokenJson(file, unsafeFileError(`JSON file cannot be a symlink: ${path.basename(file)}`)));
        return fallback;
      }
      if (error instanceof SyntaxError || error.code === 'JSON_TOO_LARGE') reportQuarantine(await quarantineBrokenJson(file, error));
