function createPreferencesRepository(deps) {
  ...
  async function getPrefs() {
    ...
    return {
      ...
      lastBackupAt: normalizeLastBackupAt(cfg.lastBackupAt),
      // Set in main-process memory by initializeConfig when a corrupt config
      // was rebuilt from the vault folders. Never persisted, so the next boot
      // reads a valid config and this is null again.
      configRecovery: typeof getConfigRecovery === 'function' ? getConfigRecovery() : null,
    };
  }

  async function recordBackupExport(value = new Date().toISOString()) {
    const at = normalizeLastBackupAt(value);
    if (!at) throw new Error('Invalid backup timestamp');
    await updateConfig(cfg => { cfg.lastBackupAt = at; });
    return at;
  }
  ...
  async function clearFeatureUsage() {
    await updateConfig(cfg => { cfg.featureUsage = null; });
  ...
  async function installCustomTheme(theme) {
    let installed;
    await updateConfig(cfg => {
      installed = themes.upsertCustomTheme(cfg.customThemes, theme);
  ...
  return {
    getPrefs, setPrefs, recordBackupExport, featureUsageStatus, recordFeatureUsage, clearFeatureUsage,
