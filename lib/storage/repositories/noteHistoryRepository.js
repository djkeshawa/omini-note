function createNoteHistoryRepository(d) {
  ...
  async function deleteNote(vaultId, noteId, snapshot = null) {
    ...
    }, snapshot.body || '', snapshot.frontMatter || '') : existing.text;
    const { meta, body, source } = d.parseFrontMatter(snapshotText);
    ...
    await d.atomicWriteFile(d.trashNoteFile(v.slug, trashId), d.serializeFrontMatter({
      ...
      pinned: !!meta.pinned,
      workflowArchived: !!meta.workflowArchived,
      originalId: id,
      trashId,
      deletedAt,
      originalTitle: meta.title || 'Untitled',
    }, body, source), 'utf8');
  ...
  async function restoreDeletedNote(vaultId, trashId) {
    const v = await vault(vaultId);
    const sourceFile = d.trashNoteFile(v.slug, d.validateTrashId(trashId));
