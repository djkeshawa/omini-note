...
// Distills a note into a memory. The note itself is not modified.
async function rememberNote(config, note, { vaultName } = {}) {
  const title = String(note?.title || 'Untitled').trim();
...
module.exports = {
  ...
  rememberNote,
  __test: { memoryNoteId, memoryNoteTitle, memoryToNote, publicMemory },
};
