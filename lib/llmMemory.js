...
// Idempotent: existing edges are fetched once and skipped so re-syncing never
// piles up duplicate relationships.
async function syncNoteLinks(config, edges, options = {}) {
  ...
  try {
    for (const rel of await listRelationships(config)) {
      existing.add(`${rel.sourceId}|${rel.targetId}|${rel.relationship}`);
    }
  } catch { /* proceed without dedup if the listing fails */ }
  ...
  for (const edge of Array.isArray(edges) ? edges : []) {
    ...
    planned.push({ source, target, rel, strength: edge.strength, evidence: edge.evidence });
    if (planned.length >= MAX_RELATIONSHIP_BATCH) break;
  }

  const result = { total: planned.length, created: 0, failed: 0, skipped: seen.size - planned.length, errors: [] };
  for (const edge of planned) {
    try {
      await addRelationship(config, {
        ...
        evidence: edge.evidence || { source: 'vispnote', reason: 'Mirrors a VispNote [[wiki-link]] between notes.' },
      });
      result.created++;
    } catch (err) {
      result.failed++;
