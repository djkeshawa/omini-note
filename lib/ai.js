...
function isPrivateHost(hostname) {
  ...
  const wildcardIp = privateIpFromWildcardDns(host);
  if (wildcardIp && isPrivateHost(wildcardIp)) return true;
  ...
  if (ipVersion === 6) {
    ...
    if (mappedIpv4 && isPrivateHost(mappedIpv4)) return true;
    return host === '::1'
      || host.startsWith('fc')
      || host.startsWith('fd')
...
function ipv4FromMappedIpv6(host) {
  const compactPrefix = '::ffff:';
  const expandedPrefix = '0:0:0:0:0:ffff:';
  const value = host.startsWith(compactPrefix)
    ? host.slice(compactPrefix.length)
    : host.startsWith(expandedPrefix)
      ? host.slice(expandedPrefix.length)
      : '';
  if (!value) return '';
...
function privateIpFromWildcardDns(host) {
  const suffixes = ['.nip.io', '.sslip.io'];
  const suffix = suffixes.find(item => host.endsWith(item));
  if (!suffix) return '';
