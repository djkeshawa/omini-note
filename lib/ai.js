...
// Own keys only. A stored provider name of `constructor` or `__proto__` hits
// Object.prototype, so the "unknown provider falls back to Ollama" guards read
// as satisfied and the AI layer then ran on a descriptor with no base URL.
...
function sanitizeConfigPatch(patch, { rejectUnknown = true } = {}) {
  ...
  for (const key of Object.keys(patch)) {
    ...
    if (URL_CONFIG_KEYS.has(key)) {
      ...
      next.provider = providerEntry(provider) ? provider : 'ollama';
    } else if (key === 'enabled') {
...
function applyConfig(config) {
  ...
  const next = { ...config };
  const provider = providerEntry(next.provider) ? next.provider : 'ollama';
  const meta = providerEntry(provider);
...
function markEmbedModelFailure(error, model = CONFIG.embedModel) {
  const failedModel = sanitizeModelName(model);
  const reason = failedModel
    ? `Embedding model "${failedModel}" does not support embeddings. Select or pull an embedding model such as nomic-embed-text.`
    : 'Embedding model is not configured.';
  if (failedModel !== CONFIG.embedModel) return reason;
  embedModelFailure = {
    model: failedModel,
    reason,
...
function currentEmbedModelFailure(model = CONFIG.embedModel) { return embedModelFailure && embedModelFailure.model === model ? embedModelFailure : null; }
async function embedWithConfiguredModel(text, opts = {}, model = CONFIG.embedModel) {
  model = sanitizeModelName(model);
  const failure = currentEmbedModelFailure(model);
  if (failure) {
  ...
  try {
    return await ollama.embed(model, text, { keep_alive: OLLAMA_KEEP_ALIVE, ...opts });
  } catch (e) {
    if (isEmbeddingUnsupportedError(e)) {
      const reason = markEmbedModelFailure(e, model);
...
function currentProviderMeta() {
  return providerEntry(CONFIG.provider) || PROVIDERS.ollama;
}
