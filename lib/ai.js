...
function lexicalScoreNote(note, query) {
...
async function executeResearchCall(call, state) {
  const { allNotes, byId, byTitle, query, signal } = state;
  ...
  if (call.tool === 'search_notes') {
    ...
    const matches = allNotes
      .map(note => ({ note, score: lexicalScoreNote(note, q) }))
      .filter(item => item.score > 0)
      .sort((a, b) => b.score - a.score || new Date(b.note.modifiedAt || b.note.date || 0) - new Date(a.note.modifiedAt || a.note.date || 0))
      .slice(0, limit)
      .map(item => item.note);
  ...
  if (call.tool === 'get_backlinks') {
    ...
    const backlinks = target
      ? allNotes
        .filter(note => note.id !== target.id && extractWikiLinkTitles(note.body || '').map(titleKey).includes(titleKey(target.title)))
        .slice(0, limit)
      : [];
  ...
  if (call.tool === 'get_notes_by_tag') {
    ...
    const tagged = tag
      ? allNotes.filter(note => (note.tags || []).map(value => String(value).toLowerCase()).includes(tag)).slice(0, limit)
      : [];
...
async function buildVaultContext(vaultId, query, st, storeApi = vaultStore, options = {}) {
  ...
  let research = null;
  if (options.recursiveResearch !== false) {
    research = await runRecursiveNoteResearch({
      query,
      allNotes,
      seedIds: expanded.map(item => item.note?.id).filter(Boolean),
      options,
    });
  }
  const selected = [
