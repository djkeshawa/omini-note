...
function isUsefulResearchSummary(summary) {
  ...
  return !/\b(error|failed|timed out|no valid notes|summary limit reached)\b/i.test(text);
}
...
function buildRagPrompt(query, context) {
  ...
  const ctx = context.notes.map((note, i) => {
    ...
    let body = String(note.body || '').trim() || '(empty note)';
...
async function editText({ text, instruction, scope, jobId, systemMessage, model, maxTokens } = {}) {
  ...
  try {
    ...
    if (!st.chatModelOk) {
      return { ok: false, error: chatUnavailableReason(st) };
    }
...
module.exports = {
  status, connect, getConfig, setConfig, previewConfig, applyConfig,
  cancelJob,
