...
function runMigrations(db) {
  ...
  if (current < 3) {
    const columns = db.prepare('PRAGMA table_info(embedding_chunks)').all();
    if (!columns.some(column => column.name === 'content_hash')) {
      db.exec('ALTER TABLE embedding_chunks ADD COLUMN content_hash TEXT');
    }
    setMeta(db, 'embeddingNormalized', false);
    setMeta(db, 'reindexNeeded', true);
    setMeta(db, 'reindexReason', 'embedding source hashes changed');
