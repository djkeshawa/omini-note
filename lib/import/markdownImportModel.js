...
// `[`, `]`, `|` and `#` are wiki-link syntax, so a title carrying one cannot be
// addressed by a link to it: the reader stops at the first such character, and
// an imported heading like `# Setup | Windows` produced `[[Setup | Windows|the
// guide]]`, which resolves to a note called "Setup " that does not exist. Every
// link to that note was dead on arrival. Import is where external text is made
// to fit the app's rules, so they come out here.
...
function allocateUniqueTitle(rawTitle, usedTitles) {
  ...
  while (usedTitles.has(candidate.toLocaleLowerCase())) {
    const marker = ` (${suffix++})`;
    candidate = `${base.slice(0, MAX_TITLE_LENGTH - marker.length).trimEnd()}${marker}`;
  }
  usedTitles.add(candidate.toLocaleLowerCase());
  return candidate;
}

function wikiLinkSourceNames(sourceTitle, filePath) {
  const names = [sourceTitle, path.basename(filePath, path.extname(filePath))]
    .map(value => String(value || '').trim())
    // These characters have structural meaning inside [[wiki links]], so an
    // apparent match containing one is ambiguous and must not be rewritten.
    .filter(value => value && !/[\[\]|#\r\n]/.test(value));
