...
test('Review fixes wire settings, rollup, reminders, and safe note paths', () => {
  ...
  assert.match(utilityPanels, /Review past days/);
  assert.match(utilityPanels, /dailyNote \? 'Open daily note' : 'Create daily note'/);
  assert.match(utilityPanels, /title="Open loops"/);
  assert.doesNotMatch(utilityPanels, /No open loops for this range/);
  assert.doesNotMatch(utilityPanels, /No reminders due in this range/);
  assert.match(utilityPanels, /label: 'Overdue'/);
  assert.match(utilityPanels, /label: 'Due today'/);
  assert.doesNotMatch(utilityPanels, /label: 'Upcoming'/);
  assert.match(utilityPanels, />Plan<\/button>/);
  assert.match(utilityPanels, /aria-label=\{`Plan \$\{actionLabel\(item\)\}`\}/);
  ...
  assert.match(sidebar, /const rollupCount = Number\.isFinite\(todayCount\) \? Math\.max\(0, todayCount\) : 0/);
