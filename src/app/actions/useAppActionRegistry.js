...
import { createActionRegistryHelpers } from './actionRegistryHelpers.js';
...
export function useAppActionRegistry(ctx) {
  ...
  const {
    ...
    openCanvasDashboard, openReferencePane, openViews, plugins, rebuildIndex, recordPhase5Metric,
  ...
  return useMemo(() => {
    ...
    const {
      appendToBody, cleanWikiTitle, integerArg, objectSchema, openView,
      resolveCanvas, resolveVault, searchNotesFast, stringArg, updateNoteTagsById,
    } = createActionRegistryHelpers({
      activeCanvas, activeVault, activeVaultId, canvases, desktopBridge,
      hasDisk: HAS_DISK, markDirty, mutations: MN_APP_MUTATIONS, navigateView,
      notesWithBody, setNotes, setSelectedTag, setSelectedWorkflow, updateNoteBody,
    ...
    const actions = [
      ...
      ...smartViewDefinitions.map(definition => ({
        ...
        description: 'Open a saved view.',
        section: 'Views',
        keywords: `view saved table board calendar ${definition.type || ''}`,
        inputSchema: objectSchema(),
        run: () => openViews(definition.id),
      })),
      {
        id: 'vault-health',
    ...
    HAS_DISK,
    MN_APP_ACTIONS_FACTORY.createRegistry,
    MN_APP_MUTATIONS,
    MN_FEATURES,
    MN_MEMORY_ACTIONS,
    activeCanvas,
